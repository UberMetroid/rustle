...
fn Cell(value: char, status: String, position: usize, is_revealing: bool, is_completed: bool, destroy_trigger: ReadSignal<String>, is_last_typed: bool, is_hard_mode: bool) -> impl IntoView {
    ...
    let classes = move || {
        ...
        if is_completed || is_revealing { if !status_c.is_empty() { base.push_str(&format!(" {}", status_c)); } else { base.push_str(" cell-neutral"); } } else { base.push_str(" cell-neutral"); }
    ...
    let aria_label = format!("Letter {}, {}", if value == ' ' { "Empty".to_string() } else { value.to_string() }, if status.is_empty() { "Unknown" } else { &status });
...
fn Row(guess: String, statuses: Vec<String>, is_revealing: bool, is_completed: bool, is_jiggling: Signal<bool>, destroy_trigger: ReadSignal<String>, last_typed_index: i32, is_hard_mode: bool) -> impl IntoView {
    let aria_label = format!("Guess row {}", if guess.is_empty() { "Empty".to_string() } else { guess.clone() });
    ...
    create_effect(move |_| {
        if let Some(storage) = get_storage() {
            if let Ok(Some(t)) = storage.get_item("color-theme") { 
                ...
                set_snarky_comment.set("PICK A FACTION (RIGHT). TOGGLE HARD MODE (LEFT).".to_string());
                set_timeout(move || set_snarky_comment.set(String::new()), std::time::Duration::from_millis(8000));
            }
