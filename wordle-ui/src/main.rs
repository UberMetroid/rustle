...
fn Cell(value: char, status: String, position: usize, is_revealing: bool) -> impl IntoView {
    ...
    let style = move || {
        format!("animation-delay: {}ms; --default-cell-bg-color: #ffffff; --default-cell-border-color: #000000; --default-cell-text-color: #000000;", delay)
    };
                ...
                view! {
                    <div class="flex justify-center mb-1 w-full">
                        {if i == 2 {
                            view! {
                                <button 
                                    class="h-14 px-4 mx-0.5 rounded font-bold bg-gray-400 text-white flex-1"
                        ...
                        {row.into_iter().map(|c| {
                            ...
                            view! {
                                <button 
                                    class=move || format!("h-14 mx-0.5 rounded font-bold text-white flex-1 min-w-[30px] {}", bg_class())
...
fn App() -> impl IntoView {
    ...
    let on_key = Callback::new(move |key: String| {
        if key == "ENTER" {
            ...
            if input.len() == 5 && is_word_in_list(&input) {
                set_guesses.update(|gs| gs.push(input));
    ...
    view! {
        <div class="flex h-screen flex-col items-center justify-between py-8 text-black dark:text-white">
            <div class="w-full max-w-[500px] flex flex-col items-center">
                <nav class="w-full flex justify-between items-center px-4 mb-8 border-b border-gray-500 pb-2">
                    <h1 class="text-3xl font-bold">"Wordle Rust"</h1>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    {move || if guesses.get().len() < 6 {
                        view! { <Row guess=current_input.get() solution=solution_data.get().solution is_revealing=false /> }.into_view()
                    } else { view! {}.into_view() }}

                    {move || (0..(5_usize.saturating_sub(guesses.get().len() + if guesses.get().len() < 6 { 1 } else { 0 }))).map(|_| {
