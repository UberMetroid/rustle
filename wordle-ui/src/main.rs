...
fn App() -> impl IntoView {
    ...
    view! {
        <div class="flex flex-col h-full bg-app-bg text-app-text overflow-hidden transition-all duration-500">
            <header class="w-full flex flex-col items-center pt-4 sm:pt-6 shrink-0 relative z-50">
            ...
            <div class="flex-1 min-h-[24px] max-h-[8vh] flex items-center justify-center pointer-events-none px-4 shrink-0 relative z-50"> {move || { let snark = snarky_comment.get(); if !snark.is_empty() { let color = if game_won.get() { "text-green-400" } else if game_lost.get() { "text-red-400" } else { "text-theme-primary" }; view! { <div key=snark.clone() class=format!("text-[10px] sm:text-xs font-black uppercase tracking-widest {} snarky-toast text-center pointer-events-auto", color)> {snark} </div> }.into_view() } else { view! {}.into_view() } }} </div>
